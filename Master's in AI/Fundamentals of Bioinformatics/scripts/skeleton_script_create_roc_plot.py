...
def calculate_coordinates(predictor_score_dict, benchmark_dict, out_filepath):
    ...
    for i in range(len(sorted_score_hgvs_pairs)):
        ...
        if benchmark_dict[hgvs] == 'Benign':
            num_benign+=1
        elif benchmark_dict[hgvs]== 'Pathogenic':
            num_pathogenic+=1
        ...
        if type_predictor == 'BLOSUM':
            if score < 0 and benchmark_dict[hgvs] == 'Benign': #false positive
                num_benign += 1
            elif score < 0 and benchmark_dict[hgvs] == 'Pathogenic': #true positive
                num_pathogenic += 1
            elif score>0:
                continue
        elif type_predictor == 'sift':
            if score < 0.6 and benchmark_dict[hgvs] == 'Benign': #false positive
                num_benign += 1
            elif score < 0.6 and benchmark_dict[hgvs] == 'Pathogenic': #true positive
                num_pathogenic += 1
            elif score>0.6:
                continue
        elif type_predictor == 'polyphen':
            if score > 0.7 and benchmark_dict[hgvs] == 'Benign': #false positive
                num_benign += 1
            elif score > 0.7 and benchmark_dict[hgvs] == 'Pathogenic': #true positive
                num_pathogenic += 1
            elif score<0.7:
                continue
        '''
        ...
        #    fpr.append((1-(num_benign/total_benign)))
        #    coordinate_score.append(score)
        #########################
        ### START CODING HERE ###
        #########################
...
def integrate(fpr, tpr):
    ...
    for cur_fpr, cur_tpr in list(zip(fpr, tpr))[1:]:
        ...
        #########################
        # Calculate AUC
        auc += (cur_fpr - last_fpr)* 0.5*(cur_tpr + last_tpr)
        #########################
        ###  END CODING HERE  ###
        #########################
        last_fpr = cur_fpr
